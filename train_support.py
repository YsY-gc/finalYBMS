...
def train_mu(net, train_iter, test_iter, optimizer, device, num_epochs):
    net = net.to(device)
    net.load_state_dict(torch.load("I:\\finalYBMS\\params\\params3\\bestparams.pth"))
    ...
    for epoch in range(num_epochs):
        ...
        for x1, x2, x3, x4, x5, x6 in tqdm_train:
            ...
            optimizer.zero_grad()
            x1p, x2p, x3p, U1, a1, a2, a3, a4, a5 = net(x1, x2, x3, x4, x5, x6)
        ...
        message = 'epoch %d,train loss %.4f, test loss %.4f, psnr %.4f,time %.1f sec, a1 %.4f, a2 %.4f, a3 %.4f, a4 %.4f, a5 %.4f\n' \
                 % (epoch + 1 , train_l_sum / batch_count, tl, psnr,time.time() - start, a1, a2, a3, a4, a5)
        file = open("I:\\finalYBMS\\log\\log5.txt",'a+')
        file.write(message)
        file.close()
        #画损失图
        ...
        if psnr > best_check_point:
            ...
            torch.save(net.state_dict(),'I:\\finalYBMS\\params\\params5\\bestparams.pth')
        
        torch.save(net.state_dict(),'I:\\finalYBMS\\params\\params5\\lastparams.pth')
...
def test_loss(test_iter, net, device):
    ...
    with torch.no_grad():
        for x1, x2, x3, x4, x5, x6 in test_iter:
            ...
            l_test = A1_loss +  A2_loss  + A3_loss
            ...
            b_psnr = (x1_psnr+x2_psnr+x3_psnr) / 3
